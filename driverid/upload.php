	  ...
	  $memail=mysqli_real_escape_string($db,$_POST['memail']);
	    $mid =mysqli_real_escape_string($db,$_POST['mid']);
	     $minstititution = mysqli_real_escape_string($db,$_POST['minstitution']);
		   ...
		   $check="SELECT * FROM idcard WHERE id='$id' ";
						  ...
						  $found=mysqli_num_rows($checks);
							  ...
							  {              $f=move_uploaded_file ($orgtmpName,'images/'.$orgName);
					                  if(isset($f)){//image is a folder in which you will save documents
                                                 $queryz = "UPDATE idcard SET Picname='$orgName' WHERE id='$id' ";
                                             $db->query($queryz) or die('Errorr, query failed to upload picture');}	
									  
									  $quer = "UPDATE idcard SET Firstname='$mfname',Sirname='$msname',Mtitle='$mtitle',Email='$memail',Staffid='$mid',Rank='$rank',Department='$minstititution' WHERE id='$id' ";
...
if(isset($_POST['addmember']))
		   ...
		   $mpassword = mysqli_real_escape_string($db,$_POST['mpassword']);
		    ...
		    if (isset($_POST["mr"]))
			    ...
			    elseif (isset($_POST["pro"]))
							   ...
							   $check="SELECT * FROM idcard WHERE Firstname='$mfname' && Sirname='$msname'";
						       $checks=mysqli_query($db,$check);
						  $found=mysqli_num_rows($checks);
							  if($found==0)
							  {
							  	...
							  	$query = "INSERT INTO idcard (Firstname,Sirname,Mtitle,Email,Staffid,Rank,Department,Online,Picname) ".
 ...
 if(isset($_POST['Valuedel'])){ 	
	 ...
	 $tutor=$_POST['Valuedel'];
 	 $querry="SELECT * FROM idcard WHERE id='$tutor' ";
                     $results=mysqli_query($db,$querry);
                    $checks=mysqli_num_rows($results);
                     if($checks!=0)
                     {
      	 	                  $querry="DELETE FROM idcard WHERE id='$tutor'";
                              $results=mysqli_query($db,$querry);
                               echo"ok"; 
				      }
 ...
 if(isset($_FILES['file2']['name'])&&$_POST['Change'])	{
				 ...
				 if($id=='')
				 {
                       ...
                       $useremail=$_COOKIE['useremail'];
                          ...
                          $sqluser ="SELECT * FROM idcard WHERE Password='$userid' && Email='$useremail'";
 	 ...
 	 $qued="SELECT * FROM Profilepictures WHERE ids='$id' ";
                    ...
                    $checks=mysqli_num_rows($resul);
                     if($checks!=0)
                     {
                     	if( move_uploaded_file ($receipttmpName, 'images/'.$receiptName)){//image is a folder in which you will save documents
...
header("location:bulk.php");
		...
		while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
		{
			...
			$sid = $filesop[6];
				 $count++;
			  if($count>1){ 
			$query = "INSERT INTO idcard (Firstname,Sirname,Mtitle,Email,Staffid,Rank,Department) ".
